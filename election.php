...
class Election {
	...
	function signBallot($ballot) {
		...
		if ($newnum === 0) {
			$prevSig = $blindedHash->toHex();
		} // TODO Think about: blindedHash correct?
	...
	function signBallots($voterReq, $blindedHashesFromDB) {
		...
		*/
		...
		foreach ($numSigned as $i => $num) {
			if ($num != $this->numSignBallots[$i] ) $this->throwException(401, "Error: the number of already acquiered signatures does not match the configured number of signatures", "from server $i, sigs acquiered $num should have acquiered " . $this->numSignBallots[$i]);
		}
		...
		if (count($allowedBallots) < 1) {
			$this->throwException(301, "Error: no non-disclosed ballots left to sign", "signBallots: " . json_encode($allowedBallots));
		}
		if (count($allowedBallots) < $this->numSignBallots[$xthserver]) $this->throwException(300, "Error: not enough non-disclosed ballots left to sign", "signBallots: left to sign: " . json_encode($allowedBallots));
	...
	function signBallotsEvent($voterReq) {
		...
		// load the blinded Hashes
		$blindedHashesFromDB = $this->db->loadBlindedHashes($this->electionId, $voterReq['voterId']);
		if (count($blindedHashesFromDB) > 1) {
			$this->throwException(301, 'Error: more than one request for ballots sent', "All requested ballots: " . print_r($blindedHashesFromDB, true));
		}
		if (count($blindedHashesFromDB) < 1) {
			$this->throwException(302, 'Error: Sign request without request for disclosing ballots', "All requested ballots: " . print_r($blindedHashesFromDB, true));
		}
		...
		if (! $verified) {
			$this->throwException(300, 'Error: Ballot verification failed. I will not sign a ballot.', "voterreq: \n" . json_encode($voterReq)) . "loaded blinded hashes: " .  json_encode($blindedHashesFromDB);
	...
	*/
	...
	function verifySig($hash, $sig, $servername) {
		...
		if ($f == -1) {
			$this->throwException(200, "Error: signature verifcation failed", "verifySig: Server $servername given in signature not found");
		}
